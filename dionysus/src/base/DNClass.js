...
dn.Class.extend = function (props) {
  ...
  // Copy getter/setter
  if(this.__getters__){
    Class.__getters__ = dn.clone(this.__getters__);
  }

  if(this.__setters__){
    Class.__setters__ = dn.clone(this.__setters__);
  }
        ...
        for (var name in prop) {
            ...
            if (isFunc) {
                ...
                if (this.__getters__ && this.__getters__[name]) {
                    ...
                    dn.defineGetterSetter(prototype, propertyName, prop[name], prop[setter] ? prop[setter] : prototype[setter], name, setter);
                }
                if (this.__setters__ && this.__setters__[name]) {
                    propertyName = this.__setters__[name];
                    for (var getterItem in this.__getters__) {
                        if (this.__getters__[getterItem] === propertyName) {
                            getter = getterItem;
                            break;
                        }
                    }
                    dn.defineGetterSetter(prototype, propertyName, prop[getter] ? prop[getter] : prototype[getter], prop[name], getter, name);
                }
            }
        }
    }
...
*/
dn.defineGetterSetter = function (proto, prop, getter, setter, getterName, setterName){
if (proto.__defineGetter__) {
    if(getter){
      proto.__defineGetter__(prop, getter);
    }
    if(setter){
      proto.__defineSetter__(prop, setter);
    }
...
for (var key in obj) {
    ...
    if (((typeof copy) === "object") && copy &&
        !(copy instanceof dn.Node) && !(copy instanceof HTMLElement)) {
        newObj[key] = dn.clone(copy);
    } else {
        newObj[key] = copy;
    }
}
...
dn.inject = function(srcPrototype, destPrototype){
